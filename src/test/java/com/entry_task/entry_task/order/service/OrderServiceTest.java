...
class OrderServiceTest {
  ...
  void createOrder_validCartItems_shouldReturnOrderResponse() {
    ...
    CartItem cartItem1 =
        TestEntityFactory.createCartItem(
            TestEntityFactory.createEmptyCart(user), product1, QUANTITY_1);

    CartItem cartItem2 =
        TestEntityFactory.createCartItem(
            TestEntityFactory.createEmptyCart(user), product2, QUANTITY_2);
    cartItem1.setId(1L);
    cartItem2.setId(2L);
    List<CartItem> cartItems = List.of(cartItem1, cartItem2);

    CreateOrderRequest request =
        new CreateOrderRequest(cartItems.stream().map(CartItem::getId).toList());
    ...
    assertEquals(
        PRODUCT_PRICE_1 * QUANTITY_1 + PRODUCT_PRICE_2 * QUANTITY_2, response.totalAmount());
  ...
  void getUserOrdersList_validRequest_shouldReturnOrderList() {
    ...
    Page<Order> orderPage =
        new PageImpl<>(List.of(order1, order2), PageRequest.of(PAGE, SIZE), TOTAL_ELEMENTS);
