...
class UserFavouriteServiceTest {
    ...
    @Test
    void setUserFavouriteByProductId_productAlreadyFavourited_shouldThrowProductAlreadyFavouritedException() {
        // Given
        ...
        when(userFavouriteRepository.findByUserIdAndProductId(customer.getId(), product.getId())).thenReturn(Optional.of(userFavourite));

        // Then
        ProductAlreadyFavouritedException ex = assertThrows(ProductAlreadyFavouritedException.class, () -> userFavouriteService.setUserFavouriteByProductId(product.getId()));
        assertEquals("Product is already in User's Favourites", ex.getMessage());
    }

    @Test
    void deleteUserFavouriteByProductId_productAlreadyNotFavourited_shouldThrowFavouriteNotFoundException() {
        // Given
        User customer = TestEntityFactory.createCustomerWithId("cutomer");
        User seller = TestEntityFactory.createSellerWithId("seller");
        Product product = TestEntityFactory.createProductWithId("product", seller);
        ...
        when(productService.getActiveProductById(product.getId())).thenReturn(product);
        when(authService.getCurrentUser()).thenReturn(customer);
        when(userFavouriteRepository.findByUserIdAndProductId(customer.getId(), product.getId())).thenReturn(Optional.empty());

        // Then
        FavouriteNotFoundException ex = assertThrows(FavouriteNotFoundException.class, () -> userFavouriteService.deleteUserFavouriteByProductId(product.getId()));
