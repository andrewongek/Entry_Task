...
class CartServiceTest {
    ...
    void addProductToCart_addingValidProductWithQuantityFour_TwoExistingProductInCart_shouldUpdateQuantityToFour() {
        ...
        CartItem cartItemResponse = TestEntityFactory.createCartItem(cart, product, existingProductQuantity);
        cartItemResponse.setId(1L);
    ...
    private void assertCartResponse(
        ...
        int expectedTotalQuantity = cartItemDtos.stream()
                .mapToInt(CartItemDto::quantity)
                .sum();

        int expectedTotalPrice = cartItemDtos.stream()
                .mapToInt(CartItemDto::subTotalPrice)
                .sum();
        ...
        Map<Long, CartItemResponse> responseById = response.items().stream()
        ...
        for (CartItemDto dto : cartItemDtos) {
            ...
            assertNotNull(
                    item,
                    "Missing CartItemResponse for cartItemId=" + dto.cartItemId()
            );
